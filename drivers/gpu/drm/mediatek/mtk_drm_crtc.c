...
static void mtk_crtc_cmdq_timeout_cb(struct cmdq_cb_data data)
{
	...
	/*if (is_project(20171) || is_project(20172) || is_project(20353) || is_project(21015) || is_project(21217))
		kill_surfaceflinger();*/
}
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if (state->base.event) {
		...
		if (drm_crtc_vblank_get(crtc) != 0) {
			...
			#ifdef OPLUS_BUG_STABILITY
			mm_fb_display_kevent("mtk_drm_crtc_atomic_begin error", MM_FB_KEY_RATELIMIT_1H, "invalid vblank:%d", drm_crtc_vblank_get(crtc));
			#endif
		}
