...
static void oplus_chg_send_info_dwork(struct work_struct *work)
{
	...
	if ((ret > 0) && (oplus_chg_debug_info.retry_cnt > 0)) {
		...
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_CHG_SLOW_BATT_NON_AUTH, OPLUS_NOTIFY_SC8517_ERROR);
		...
		memset(oplus_chg_debug_info.sc8547_error_reason,0,sizeof(oplus_chg_debug_info.sc8547_error_reason));
...
static void oplus_chg_print_debug_info(struct oplus_chg_chip *chip)
{
		...
		oplus_chg_debug_notify_flag_is_set(OPLUS_NOTIFY_CHARGER_INFO))
	{
		...
		if (voocphy_chip && voocphy_chip->voocphy_bidirect_cp_support) {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					"VOOCPHY[%ld, %ld, %ld, %ld, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d], ",
					voocphy_chip->cpu0_freq, voocphy_chip->cpu1_freq, voocphy_chip->cpu2_freq, voocphy_chip->cpu3_freq, voocphy_chip->ap_disconn_issue,
					voocphy_chip->disconn_pre_ibat, voocphy_chip->disconn_pre_ibus, voocphy_chip->disconn_pre_vbat, voocphy_chip->disconn_pre_vbat_calc, voocphy_chip->disconn_pre_vbus,
					voocphy_chip->r_state, voocphy_chip->vbus_adjust_cnt, voocphy_chip->voocphy_cp_irq_flag,
					voocphy_chip->voocphy_enable, voocphy_chip->slave_voocphy_enable, voocphy_chip->voocphy_iic_err, voocphy_chip->slave_voocphy_iic_err,
					voocphy_chip->voocphy_vooc_irq_flag, voocphy_chip->irq_rcvok_num, voocphy_chip->ap_handle_timeout_num, voocphy_chip->rcv_date_err_num, voocphy_chip->rcv_done_200ms_timeout_num,
			...
			if (oplus_chg_debug_info.sc8547_error_reason[0] > 0) {
				ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					", SC8517[%s, ", oplus_chg_debug_info.sc8547_error_reason);
				...
				ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					"%d, %d, %d, %d, %d]",
					voocphy_chip->master_cp_ichg, voocphy_chip->slave_cp_ichg, voocphy_chip->master_error_ibus, voocphy_chip->slave_error_ibus, voocphy_chip->error_current_expect);
			}
		}
...
void oplus_chg_sc8547_error( int report_flag, int *buf, int len) {
	...
	if (!oplus_chg_debug_notify_type_is_set(OPLUS_CHG_DEBUG_NOTIFY_TYPE_SC8517)) {
		oplus_chg_set_notify_type(OPLUS_CHG_DEBUG_NOTIFY_TYPE_SC8517);
		if (!oplus_chg_debug_notify_flag_is_set(OPLUS_NOTIFY_SC8517_ERROR)) {
			oplus_chg_set_chg_flag(OPLUS_NOTIFY_SC8517_ERROR);
			strcpy(oplus_chg_debug_info.flag_reason,
				oplus_chg_debug_notify_policy[OPLUS_NOTIFY_SC8517_ERROR].reason);
