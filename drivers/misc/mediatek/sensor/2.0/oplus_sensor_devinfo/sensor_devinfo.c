...
#define SAR_NUM                     (0x0B)
#define IS_SUPPORT_MTK_CALI         (0x0C)
#define CALI_ADAPT_Q                (0x0D)
#define ACC_GYRO_NAME               (0x0E)
...
struct delayed_work oplus_notifier_work;
...
atomic_t g_lcd_state;
struct delayed_work lcd_state_work;
...
static void sync_lcd_state_to_scp(struct work_struct *dwork)
{
    int err = 0;
    unsigned int len = 0;
    union SCP_SENSOR_HUB_DATA lcd_state_data;

    lcd_state_data.req.sensorType = 0;
    lcd_state_data.req.action = OPLUS_ACTION_SET_LCD_STATE;
    lcd_state_data.req.data[0] = atomic_read(&g_lcd_state);

    len = sizeof(lcd_state_data.req);
#ifdef CONFIG_OPLUS_SENSOR_MTK68XX
    err = mtk_nanohub_req_send(&lcd_state_data);
#else
    err = scp_sensorHub_req_send(&lcd_state_data, &len, 1);
#endif
    if (err < 0 || lcd_state_data.rsp.action != OPLUS_ACTION_SET_LCD_STATE) {
        DEVINFO_LOG("fail! err %d\n", err);
        return;
    }
}
...
static ssize_t mag_data_int2str(char *buf)
{
    ...
    {
        sprintf(node, "/odm/msensor_%d", i + 1);
        nodeoffset = fdt_path_offset(fdt, node);
        if (nodeoffset < 0) {
            DEVINFO_LOG("get %s nodeoffset fail i:%d", node, i);
            continue;
        }
        ...
        if (prop) {
            ...
            if (MAG_PARA_NUM == len / MAG_PARA_OFFSET) {
                for (j = 0; j < MAG_PARA_NUM; j++) {
                    value = fdt32_to_cpu(*(data_addr + (2 * j)));
                    symble = fdt32_to_cpu(*(data_addr + (2 * j + 1))) > 0 ? -1 : 1;
...
static int sensor_feature_read_func(struct seq_file *s, void *v)
{
    ...
    case GOLD_REAR_CCT_6K:
        ...
        break;
    case ACC_GYRO_NAME:
        DEVINFO_LOG("acc_gyro_name = %s \n", sensorlist_info[gyro].name);
        seq_printf(s, "%s", sensorlist_info[gyro].name);
        break;
...
static int oplus_sensor_feature_init()
{
    ...
    p_entry = proc_create_data("acc_gyro_name", S_IRUGO, oplus_sensor, &Sensor_info_fops,
            UINT2Ptr(ACC_GYRO_NAME));
    if (!p_entry) {
        DEVINFO_LOG("acc_gyro_name err\n");
        return -ENOMEM;
    }
    ...
    if (evdata && evdata->data) {
        if (event == FB_EARLY_EVENT_BLANK) {
            ...
            if (blank == FB_BLANK_UNBLANK) {
                atomic_set(&g_lcd_state, 1);
                DEVINFO_LOG("lcd on %d\n", atomic_read(&g_lcd_state));
                schedule_delayed_work(&lcd_state_work, 0);
            } else if (blank == FB_BLANK_POWERDOWN) {
                atomic_set(&g_lcd_state, 0);
                DEVINFO_LOG("lcd off %d\n", atomic_read(&g_lcd_state));
                schedule_delayed_work(&lcd_state_work, 0);
            }
        }
    }
...
static void oplus_notifier_register(struct work_struct *dwork)
{
#ifdef CONFIG_OPLUS_SENSOR_MTK68XX
    ...
    DEVINFO_LOG("scp_A_register_notify end\n ");
#endif
}
...
static int __init sensor_devinfo_init(void)
{
    ...
    INIT_DELAYED_WORK(&lcd_state_work, sync_lcd_state_to_scp);

    /*init parameter*/
    INIT_DELAYED_WORK(&parameter_work, sensor_devinfo_work);
    schedule_delayed_work(&parameter_work, msecs_to_jiffies(100));
    INIT_DELAYED_WORK(&oplus_notifier_work, oplus_notifier_register);
    schedule_delayed_work(&oplus_notifier_work, msecs_to_jiffies(100));
    atomic_set(&g_lcd_state, 1);
    return 0;
}
