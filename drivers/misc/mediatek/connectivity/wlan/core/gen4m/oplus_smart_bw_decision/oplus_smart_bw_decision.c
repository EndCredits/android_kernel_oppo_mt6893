...
signed int htKalWCNKeyLogWrite(IN const char *log, IN enum WCN_KEY_LOG_MODE mode)
{
        ...
        //2021/2/23: modify for log adjustment, won't return TYPE_LOG_SCAN_RSSI_RATIO "scan_rssi_ratio"
        //TYPE_LOG_SCAN_RSSI_RATIO, log purpose:
        //Each time when connect to AP, when the WIFI KO is reloaded or the distance from the last log > 30s,
        //it will return the connected AP's ssid, rssi and the most 25 active AP's ssid, rssi.
        //The HT40 connected AP's SSID RSSI can be found in TYPE_LOG_CONNECT_BAND_RATIO.
        //So this log type seems useless both from debug and log sumamry info
        if (mode == TYPE_LOG_SCAN_RSSI_RATIO) {
                return 0;
        }
        //end
